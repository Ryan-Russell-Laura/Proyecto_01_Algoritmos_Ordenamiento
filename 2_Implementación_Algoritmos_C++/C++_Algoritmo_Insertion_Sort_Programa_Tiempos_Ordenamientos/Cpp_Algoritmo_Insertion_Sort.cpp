...
void insertionSort(vector<int>& numeros_aleatorios) {
    int n = numeros_aleatorios.size();

    for(int i=1;i<n;i++){
        int key=numeros_aleatorios[i];
        int j=i-1;

        // Mover los elementos mayores que la clave hacia adelante
        while(j>=0 && numeros_aleatorios[j]>key){
            numeros_aleatorios[j+1]=numeros_aleatorios[j];
            j--;
        }

        numeros_aleatorios[j+1]=key;
    }
}
...
int main(){
	...
	ofstream archivo_tiempo("Tiempos_Insertion_Sort_C++.txt");
    for (int tamano:tamanos){
		...
		tiempo_inicio=clock();
        insertionSort(numeros_aleatorios);
		tiempo_final=clock();
